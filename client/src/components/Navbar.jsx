...
export default function Navbar({ activeTab, setActiveTab, isCollapsed, setIsCollapsed, user }) {
  ...
  return (
    <aside
      className={`flex h-full flex-shrink-0 flex-col rounded-[28px] border border-[#ece7ef] bg-white/90 p-2 shadow-[0_20px_80px_-44px_rgba(15,23,42,0.3)] backdrop-blur-xl transition-all duration-300 ${isCollapsed ? 'w-[78px]' : 'w-[240px]'
        }`}
    >
        ...
        {filteredNavItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`group relative flex w-full items-center rounded-2xl px-3 py-2.5 text-sm font-medium transition-all duration-200 ${isCollapsed ? 'justify-center' : 'justify-start gap-3'
                } ${isActive
